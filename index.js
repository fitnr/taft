...
class Taft {
     ...
     */
    _applyLayout(layout, content, options) {
     ...
     */
    _createTemplate(file, options) {
        const source = gm.read(file, {
                strict: true
            }),
            context = source.data || {},
        ...
        return (function(pageData, preferGlobal) {
            ...
            const template = this.Handlebars.compile(page, {
                knownHelpers: this._helpers
            });
            const newTemplate = new Content(template(tplData), tplData);

            return this._applyLayout(tplData.layout, newTemplate, {
